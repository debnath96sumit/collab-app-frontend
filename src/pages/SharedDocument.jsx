...
import { ShieldX, LogIn, LayoutDashboard, HelpCircle, Shield } from 'lucide-react';
...
const SharedDocument = () => {
    ...
    useEffect(() => {
        const fetchSharedDocument = async () => {
            try {
                const response = await DocumentAPI.getSharedDocument(shareToken);
                if (response.data?.id) {
                    navigate(`/board/${response.data.id}`, { replace: true });
                } else {
                    setStatus('not-found');
                }
            } catch (error) {
                ...
                if (statusCode === 403) {
                    ...
                    setErrorMessage(error?.response?.data?.message || error?.message || 'This document is restricted.');
                } else {
                    setStatus('not-found');
                }
            }
            ...
            <div className="min-h-screen bg-background text-on-surface flex flex-col overflow-hidden">
                ...
                <main className="flex-1 relative flex items-center justify-center p-6">
                    ...
                    <div className="relative z-10 flex flex-col items-center text-center max-w-xl">
                        {/* Icon */}
                        <div className="w-24 h-24 mb-8 rounded-2xl bg-error/10 flex items-center justify-center ring-1 ring-error/20 shadow-2xl shadow-error/5">
                            <ShieldX size={48} className="text-error" strokeWidth={1.5} />
                        </div>
                        ...
                        <p className="text-lg text-on-surface-variant leading-relaxed mb-10">
                            {errorMessage || 'This document is restricted. Request access from the owner.'}
                        </p>
